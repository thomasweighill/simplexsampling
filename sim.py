...
df = pd.read_csv("./DOJBVAP_2000.csv", header=None)
for x in range(4000,100001,2000):
    df = pd.concat([df, pd.read_csv("./DOJBVAP_{0}.csv".format(x), header=None)], ignore_index=True)
...
#plt.ylabel("DOJ BVAP")
#plt.savefig("va_bvap_ensemble.png")
#plt.show()
...
plt.savefig("va_bvap_gap_histogram.png")
plt.show()
...
plt.hist(left_district, bins=[x - .5 for  x in range(1,12)])
